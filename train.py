        ...
        if epoch > params["Epoch"] - 1:
            ...
            if len(env.pursuit_vehs.keys()) != params["num_pursuit"]:
                ...
                if training_mode and last_observation_state is not None:
                    ...
                    # mixing_net.save_exp(last_action_prob,last_observation_state,sum(rewards))
                    # mixing_net.train()
                ...
                if stop_:
                    ...
                    finally_epoch = epoch
                    break
                action_prob,joint_action = controller.policy(observation_state, training_mode)
        ...
        if epoch > params["Epoch"] - 1:
            ...
            if len(env.pursuit_vehs.keys()) != params["num_pursuit"]:
                ...
                if stop_:
                    env.reset()
                    break
                action_probs,joint_action = controller.policy(observation_state, training_mode=False)
                # joint_action = np.array([0, 0, 0, 0])
                joint_action = np.array(joint_action)
                env.pursuitVehControl(commands=joint_action)
                env.evadeVehControl(choice_random=True)
